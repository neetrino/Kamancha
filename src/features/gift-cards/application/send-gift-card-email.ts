...
export async function sendGiftCardEmail(input: {
  ...
  const text = [
    ...
    "",
    `${input.purchaserName} sent you a White Shop gift card.`,
    `Value: ${amountLabel}`,
    `Code: ${input.code}`,
  ...
  const html = `
    ...
    <p><strong>${escapeHtml(input.purchaserName)}</strong> sent you a White Shop gift card.</p>
  ...
  return getProviders().email.send({
    to: input.to,
    subject: `Your White Shop gift card — ${input.code}`,
    text,
    html,
  });
}
